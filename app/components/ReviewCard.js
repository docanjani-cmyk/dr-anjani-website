 ...
 *
 * `mobileLimit` caps how many show on a phone, where cards stack one per row
 * and a long set turns into an enormous scroll. The rest stay in the DOM —
 * hidden with CSS, not removed — so nothing is lost to crawlers, and a button
 * reveals them. From md up every card shows regardless.
 */
export function ReviewGrid({ reviews, columns = 'md:grid-cols-2 lg:grid-cols-3', reveal = false, mobileLimit = 0 }) {
  const [expanded, setExpanded] = useState(false)
  const capped = mobileLimit > 0 && !expanded && reviews.length > mobileLimit
  const hiddenCount = reviews.length - mobileLimit
  ...
  return (
    <>
      <div className={`grid ${columns} gap-5`}>
        {reviews.map((t, i) => {
          const classes = [
            reveal ? `reveal reveal-delay-${(i % 3) + 1}` : '',
            capped && i >= mobileLimit ? 'hidden md:flex' : '',
          ].filter(Boolean).join(' ')
          return <ReviewCard key={`${t.name}-${i}`} review={t} className={classes} />
        })}
      </div>
